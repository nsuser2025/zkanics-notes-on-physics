#### ATAT インストール
<p>
事前に$(HOME)/ATAT/3.50/binのディレクトリを作っておく.</br>
...
#### MAPS
<p>
ATAT の MIT Ab-initio Phase Stability（MAPS）ではクラスター選択アルゴリズム
により空間的にコンパクトなクラスターを優先的に選び出す. ある多体クラスター
をクラスター展開に用いる場合は, その低次のクラスター（サブクラスター）
が全て含まれていることが条件. MAPS以外にも遺伝的アルゴリズムを用いる手法などがある.</br>
...
#### SQS
<p>
SQS（special quasirandom structure）とは, 無秩序合金の構造を生成するための数学的モデル.
SQS はランダムに生成された合金と類似した特定の統計的性質を示す一方で, 何らかの規則性やパターンを内包している。
...
.5 .0 .5</br>
.5 .5 .0</br>
.0 .0 .0 Ni=.25,Fe=.25,Cr=.25,Co=.25</br>
</p>

<p>
L1$_{2}$ $\cdots$ FCC構造の規則合金:</br>
1 1 1 90 90 90</br>
1 0 0</br>
0 1 0</br>
0 0 1</br>
...
<p>
rndstr.in ファイルの準備ができたら、最初のコマンドを実行します： </br>
corrdump -l=rndstr.in -ro -noe -nop -clus -2=1.1 </br>
...
<p>
mcsqsは、定義されたスーパーセル（巨大な計算単位）のサイズ内で、
モンテカルロ法を用いてSQSを探索します。  
</p>
...
<p>
それでは、私たちが求めているSQSそのものや、その特性を与えてくれる mcsqs の出力ファイルに注目しましょう。
mcsqs コードは5つのファイル（bestsqs-2.out や bestcorr-2.out などを数に含めればそれ以上）を生成します。
...
それでは、残りの3つのファイルに焦点を当てていきましょう。  
</p>

#### The bestsqs.out file

<p>
これは SQS（構造データ）が保存されるファイルです。
...
<p>
bestcorr.out ファイルには、ターゲットとなる合金（理想的なランダム状態）と、
それに対応する bestsqs.out の相関関数が含まれています。上記（原文）はその一例です。
...
bestcorr.out の最終行には、そのSQSの目的関数が表示されます。ATATのマニュアルに詳細な数式の説明がありますが、
一般的にこのパラメータはSQSの良さを普遍的なスケールで表したものです。
この値が負に大きければ大きいほど（よりマイナスであれば）、そのSQSは優れていると言えます。
ただし、すべての相関関数がターゲットと完全に一致した場合は、目的関数は 『Perfect_match』 と表示されます。  
</p>

#### The mcsqs.log file

<p>
mcsqs.log ファイルには、新しい情報が含まれているわけではありません。
このファイルには、コードが新しいSQSを見つけるたびに、
その目的関数（objective function）と bestcorr.out の第5列にあるすべての数値が保存されます。
すべての重要な結果がこのファイルに集約されているため、計算の全体像を把握するのに適した場所です。  
</p>

#### How to tell if a SQS is good?

<p>
bestcorr.out ファイルには SQS の良さを評価する正確な数値が記載されていますが、
その SQS が研究にとって「本当に十分な質か」を判断するのは、より複雑な問題です。
例えば、目的関数の値が -0.95 の SQS は、-0.85 のものより優れていると言えます。
しかし、前者の生成に3週間かかり、後者がわずか5日で済んだとして、結果的に両者とも「十分な質」である可能性もあります。
「これを超えれば十分」という明確な境界線があれば素晴らしいのですが、
その線は対象とする材料や、各研究で注目している物性によって異なります。
したがって、「この数値ならOK」という偽りの基準を示す代わりに、それが良い SQS であることを確認する方法についてお話ししましょう。
